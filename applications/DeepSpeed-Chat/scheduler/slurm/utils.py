...
from .cluster import node_name_is_node_type
...
def available_hostnames(
    ...
    return list(filter(lambda x: node_name_is_node_type(x, node_type), valid_hostnames))
...
def get_all_node_resources() -> Dict[str, SlurmResource]:
    ...
    for node in nodes:
        ...
        ctres.gpu_type = atres.gpu_type = cluster_spec.gpu_type_from_node_name(node_name)
        rres = ctres - atres
        if rres.valid():
